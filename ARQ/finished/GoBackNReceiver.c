...
void sendAck(int s, GoBackNMessageStruct* packet, long expected) { // set expected to the packet number wie want have next!!
  GoBackNMessageStruct* ack = allocateGoBackNMessageStruct(0);
  ack->seqNo = -1;
  ack->seqNoExpected = expected;  // set to Sequence number
  ack->size = sizeof(*ack);
  ack->crcSum = 0; // check sum
...
int main(int argc, char** argv) {
  ...
  while (1) {
    ...
    /* YOUR TASK:
     * - Check packet for errors (crcValid)
     * - Check if packet is
    ...
    if(crcValid == true){
      DEBUGOUT("SEQNO #%d  -------- lastReceivedSeqNo %ld \n", data->seqNo, lastReceivedSeqNo);

      if(data->seqNo > lastReceivedSeqNo + 1 || data->seqNo <= lastReceivedSeqNo){   /* invalid seqNo -> find out what is wrong */
          DEBUGOUT(" Something wrong  too new  or to old SEQNO->%d ------ LASTREC %ld\n", data->seqNo, lastReceivedSeqNo);
          sendAck(s, data, lastReceivedSeqNo + 1);
                        
      } else {

                DEBUGOUT("Write back to sender #%d\n", data->seqNo);
                lastReceivedSeqNo = lastReceivedSeqNo + 1;
                  //send ACK to the sender with next message I expected
                  sendAck(s, data, lastReceivedSeqNo);
                  //write correct data to the file
                  writeBuffer(output, data);
                  
                  
                  goodBytes = goodBytes + bytesRead;  //- sizeof(*data));

                   if(data->size == sizeof(*data)){
                      fclose(output);
                      printf("Totalbytes %zu and GoodBytes %zu \n", totalBytes, goodBytes); 
                      freeGoBackNMessageStruct(data);
                      break; 
                   }
      }
