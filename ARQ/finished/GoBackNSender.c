...
void initialize(int argc, char** argv) {
  ...
  while (1) {
    ...
    int c = getopt_long(argc, argv, "t:w:r:h", long_options, NULL);  // parsing of arguments from command-line
    if (c == -1) break;
...
int main(int argc, char** argv) {
  ...
  // read file
  long veryLastSeqNo = readFileIntoBuffer();  // LastACKSeq is a counter for sending data which approved with Ack  <-> veryLast is fix
  ...
  int s = udp_connect(remoteName, remotePort); // Set remote UDP peer
  if (s < 0) {
    exit(1);
  }

  while ((lastAckSeqNo + 1) < veryLastSeqNo) { // while all SeqNo will be send -> veryLast (was written from file)
    DEBUGOUT("nextSendSeqNo: %ld, lastAckSeqNo: %ld veryLastSeqNo: %ld\n", nextSendSeqNo,
             lastAckSeqNo, veryLastSeqNo);
    ...
    if (FD_ISSET(s, &readfds)) {
       ...
       */
      DataPacket *dataPacketToSent;
      //check on correct ACK

      if((ack->seqNoExpected - 1) > lastAckSeqNo && crcValid == true){ // all send messages get ACK
        //DEBUGOUT("SEQEXP -> %d, lastSEQ -> %ld \n", ack->seqNoExpected - 1, lastAckSeqNo);
        //free buffer from already send data
        //set lastAckSeqNo on last Ack-message
        lastAckSeqNo = ack->seqNoExpected - 1;
        nextSendSeqNo = ack->seqNoExpected; 
        freeBuffer(dataBuffer, getFirstSeqNoOfBuffer(dataBuffer), lastAckSeqNo); 

        //Recalculate timerExpiration
        //DEBUGOUT("TAKE PACKET NR %d \n", ack->seqNoExpected);
        dataPacketToSent = getDataPacketFromBuffer(dataBuffer, ack->seqNoExpected);

        if(dataPacketToSent) {
          timerExpiration = dataPacketToSent->timeout;
        }
      }
      /* END YOUR TASK */
    ...
    if (timercmp(&timerExpiration, &currentTime, < )) {
      ...
      //set nextSendSeqNo on last ACK message from receiver
    ...
    if (FD_ISSET(s, &writefds)) {
      while (nextSendSeqNo - lastAckSeqNo < window && nextSendSeqNo <= veryLastSeqNo) { // max number of packet should be smaller then window
        ...
        if(timercmp(&currentTime, &timerExpiration, >=)){
          // set to null and begin sending from first msg 
          nextSendSeqNo = 0; 
          timerExpiration.tv_sec = LONG_MAX;
        }
